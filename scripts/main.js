let mapFrance = L.map('mapFrance').setView([46.492563, 2.719138], 5.34);
...
for (let i = 0; i < places.length; i++) {
  L.marker([places[i].lat, places[i].long]).bindPopup('<h3>' + places[i].placeName + '</h3>' + '<p>' + places[i].desc + '</p>').addTo(mapFrance);
}
